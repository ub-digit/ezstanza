<template>
    <div v-if="visible" :class="containerClass" :aria-label="label" v-bind="ptm('root')">
        ...
        <slot v-if="removable" name="removeicon" :onClick="onClick" :onKeydown="onKeydown">
            <component :is="removeIcon ? 'span' : 'TimesCircleIcon'" tabindex="0" :class="['p-chip-remove-icon', removeIcon]" @click="onClick" @keydown="onKeydown" v-bind="ptm('removeIcon')"></component>
        </slot>
    </div>
</template>
