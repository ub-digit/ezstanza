defmodule Ezstanza.TagsRelationship do
  ...
  def find_or_create_tags(_repo, nil, _user_id), do: {:ok, []}
  def find_or_create_tags(_repo, [], _user_id), do: {:ok, []}
  def find_or_create_tags(_repo, tags, _user_id) when not is_list(tags) do
    {:error, :tags_is_not_a_list}
  end

  def find_or_create_tags(repo, tags, user_id) do
    with {:ok, {tag_ids, tag_names}} = attr_tags_split_ids_names(tags) do
      ...
      tags = tags ++ Enum.map(tag_names, fn tag_name ->
        repo.insert!(
          %Tag{name: tag_name, user_id: user_id},
